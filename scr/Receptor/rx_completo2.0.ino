...
uint8_t thisAddress = 0x25; // end. oficial do RX
uint8_t idDestino = 0x24;

uint8_t buf[RH_ASK_MAX_MESSAGE_LEN];
uint8_t buflen = sizeof(buf);

uint8_t from = 0x24; //0x24 (ByPass) para receptor e 0x23 (TX) para bypass
...
void loop() {
    ...
    if (driver.recv(buf, &buflen)) {
      ...
      Serial.println (driver.headerFrom()); // Printa o remetente
      
      // Delimita rementente
      if(driver.headerFrom() == from){
    ...
    strncpy((char*)verificacao, (char*)buf, strlen((char*)buf));
